...
function WaveformBars({ bars, accent, active }) {
  ...
  return (
    <div className="inline-flex h-14 items-center gap-[5px]">
      {data.map((level, index) => {
        const height = active ? Math.max(10, Math.round(14 + level * 28)) : 9
        return (
          <motion.span
            key={index}
            animate={{ height, opacity: active ? 1 : 0.38 }}
            transition={{ type: 'spring', stiffness: 420, damping: 18, mass: 0.36 }}
            className={`w-[5px] rounded-full bg-gradient-to-t ${cfg.bar}`}
          />
        )
      })}
    </div>
      ...
      <AnimatePresence mode="wait">
        {state === 'listening' && (
          <motion.div
            ...
            className={`relative inline-flex min-h-[64px] items-center gap-3 rounded-full border border-white/10 ${cfg.bg} px-4 py-3 backdrop-blur-md ${cfg.glow}`}
          >
            <div className="relative flex h-11 w-11 items-center justify-center rounded-full">
              ...
              <motion.div
                ...
                className="absolute h-4 w-4 rounded-full bg-white/90 shadow-[0_0_16px_rgba(255,255,255,0.45)]"
              />
            </div>
        ...
        {state === 'processing' && (
          <motion.div
            ...
            exit={{ opacity: 0, scale: 0.84, y: -4 }}
            transition={{ duration: 0.2 }}
            className={`inline-flex min-h-[52px] items-center gap-3 rounded-full border border-white/10 ${cfg.bg} px-4 py-2.5 backdrop-blur-md ${cfg.glow}`}
          >
            <motion.div
              animate={{ scale: [0.92, 1.08, 0.92] }}
              transition={{ duration: 1.1, repeat: Infinity, ease: 'easeInOut' }}
              className={`h-8 w-8 rounded-full bg-gradient-to-br ${cfg.orb}`}
            />
        ...
        {state === 'idle' && (
          <motion.div
            ...
            className="inline-flex min-h-[46px] items-center gap-2 rounded-full border border-white/10 bg-black/25 px-3 py-2 backdrop-blur-md"
          >
            <div className={`h-7 w-7 rounded-full bg-gradient-to-br ${cfg.orb} opacity-55`} />
